...
<div class="row">
  ...
  <div class="col-lg-6 col-md-12 col-sm-12 custom_container">
    <h3 class="text-center theme_green fw-bold mt-2 display-5">{{__('register')}}</h3>
    <x-alert />
    <form action="{{ route('register.store') }}" method="POST" class="px-4 py-3">
      @csrf
      <div class="mb-3">
      ...
      <div class="pt-3 mt-4" style="width: fit-content; margin:0 auto">
        <p>{{__('has_account')}} <a class="custom_link_auth" href="{{ route('login.create') }}">{{__('login_account')}}</a>
        </p>
      </div>
    </form>
  </div>
