        ...
        <div class="row">
            ...
            <div class="col">
                <div class="text-center">
                    @for( $i = 0 ; $i < $build_type_count ; $i++ ) @php($build_type=$build_types[$i])
                    @if(request()->fullUrl()=='http://localhost:8000/build_type/buy/post?build_type_id='.$i)
                    <a href="{{ route('build_type.buy.post.index','&build_type_id='.$i) }}" class="text-decoration-none h3 mx-3 mt-5 " style="color: #12ca8a; border-bottom: 2px solid #12ca8a;">{{ __('buy_post') }}</a>
                    <a href="{{ route('build_type.sale.post.index','&build_type_id='.$i) }}" class="text-decoration-none h3 mx-3 mt-5" style="color: #12ca8a">{{ __('sale_post') }}</a>
                    @endif
                    @endfor
                </div>
            </div>
    ...
    <div class="bg-light rounded p-4 mb-5">
        ...
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-2 row-cols-lg-3  row-cols-xl-3 g-4 p-2">
            ...
            <div class="mb-2">
                ...
                <x-card_lg purpose="buy" :route="route('buy.post.show', $post->id)" saleProfile="buy" :$post :$users :$profile_image/>
            </div>
            @endforeach
            <!-- End Columns --->
        </div>
